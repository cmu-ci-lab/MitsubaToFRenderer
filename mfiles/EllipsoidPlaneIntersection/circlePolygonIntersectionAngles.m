...
for i=1:length(points)
    ...
    if(epsLesser(norm_p(i), r))
        locations_p(i) = -1;
        checks = checks + 1;
    end
    if(epsGreater(norm_p(i), r))
        locations_p(i) =  1;
        checks = checks + 1;
    end
    if(isnan(locations_p(i)))
...
for i=1:length(points)
    ...
    if(locations_p(i) == -1 && locations_p(j) == -1) %% both points inside, do nothing
        continue;
    end
